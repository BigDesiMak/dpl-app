...
// CHANGE 3: 8 players total — 6 male + 2 female, no substitutes
const BUDGET    = 10000
const MAX_TOTAL = 8
const MAX_MALE  = 6
const MAX_FEMALE = 2
...
export default function TeamSelection() {
  ...
  async function loadData() {
    ...
    const [{ data: players }, { data: teams }, { data: ft }] = await Promise.all([
      ...
      // CHANGE 4: each phase gets its own team — just load for this phase
  ...
  const filteredPlayers = useMemo(() => {
    ...
    return list.sort((a, b) => {
      ...
      // default: selected first then alpha
      const aS=selectedIds.includes(a.id)?0:1, bS=selectedIds.includes(b.id)?0:1
  ...
  function togglePlayer(player) {
    ...
    if (selectedIds.includes(player.id)) {
      ...
      if (!canSelect(player)) {
        if (selectedIds.length >= MAX_TOTAL) toast.error(`Max ${MAX_TOTAL} players allowed`)
  ...
  async function saveTeam() {
    ...
    try {
      ...
      // CHANGE 3: all 8 players are playing — no subs
    ...
    <Layout>
      <div className="page-content">
        <div className="empty-state" style={{paddingTop:80}}>
          ...
          <p className="empty-state-desc">Team selection opens when admin activates a phase.</p>
        </div>
      </div>
    </Layout>
  )
    ...
    <Layout>
      <div className="page-content">
        <div className="page-header">
          ...
          <p className="page-subtitle">{phase.name} · Budget: ₹{BUDGET.toLocaleString()} · 8 players (6M + 2F)</p>
        </div>

        {/* CHANGE 4: Phase notice */}
        <div style={{
          ...
          📅 Each phase has its own independent team. You can create or edit your team freely for <strong style={{color:'var(--gold-400)'}}>{phase.name}</strong>.
        </div>
        ...
        {activeTab==='lineup' && (
          <div>
            <div className="form-group">
              ...
              <input className="form-input" style={{maxWidth:360}} placeholder="e.g. Thunder Eagles" value={teamName} onChange={e=>setTeamName(e.target.value)}/>
            </div>

            {/* All 8 selected players */}
